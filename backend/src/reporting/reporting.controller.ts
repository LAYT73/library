import { Controller, Get, Query, Res, Param, Post, Body, UseGuards } from '@nestjs/common';
...
export class ReportingController {
  ...
  async fund(@Query('format') format: string, @Res() res: Response) {
    ...
    return res.send(out.data as any);
  }
  ...
  async coverage(@Param('disciplineId') disciplineId: string, @Query('format') format: string, @Res() res: Response) {
    const fmt = (format || 'csv').toLowerCase();
